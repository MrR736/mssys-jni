...
#define base_c_cast(t,p)	((t)(long long)(p))
#define ubase_c_cast(t,p)	((t)(unsigned long long)(p))
#elif defined(__LP64__) || defined(_LP64__) || (defined(__WORDSIZE) && __WORDSIZE == 64)
#define base_c_cast(t,p)	((t)(long)(p))
#define ubase_c_cast(t,p)	((t)(unsigned long)(p))
#else
#define base_c_cast(t,p)	((t)(int)(p))
#define ubase_c_cast(t,p)	((t)(unsigned int)(p))
#endif
...
#define _CallStaticObjectMethod(env, ex, methodID) env->CallObjectMethod(ex, methodID)
#define _CallStaticObjectMethod_l(env, ex, methodID, ...) env->CallObjectMethod(ex, methodID, __VA_ARGS__)
...
#define _CallStaticObjectMethod(env, ex, methodID) (*env)->CallObjectMethod(env, ex, methodID)
#define _CallStaticObjectMethod_l(env, ex, methodID, ...) (*env)->CallObjectMethod(env, ex, methodID, __VA_ARGS__)
...
#define _GetStaticDoubleField(env, clazz, fieldID) (*env)->GetStaticStaticDoubleField(env, clazz, fieldID)
...
#if defined(_WIN32) || defined(_WIN64)
#define BASE_EXPORT __declspec(dllimport) BASE_VISIBILITY(default)
#define BASE_IMPORT __declspec(dllexport) BASE_VISIBILITY(default)
#define BASE_CALL __stdcall
#else
#define BASE_EXPORT BASE_VISIBILITY(default)
#define BASE_IMPORT BASE_VISIBILITY(default)
#define BASE_CALL
#endif

#define BASE_EXPORTC BASE_EXTERN_C BASE_EXPORT
#define BASE_IMPORTC BASE_EXTERN_C BASE_IMPORT
...
#define base_char_cast(ptr) ((char*)(ptr))

#define BASE_THROWJAVA(t,Class)\
static inline void t(JNIEnv *env, const char* mes) {\
    jclass cls = _FindClass(env,Class);\
    if (cls == NULL) {\
	BASE_LOGE("Unable to find exception class: %s\n",Class);\
	_ExceptionClear(env);\
    } else {\
	_ThrowNew(env,cls,mes);\
	_DeleteLocalRef(env, cls);\
    }\
}

#ifndef THROWJAVA
#define THROWJAVA BASE_THROWJAVA
#endif

BASE_EXTERN_START

#define DEFINE_ARRAY_FIELD_GETTERS_SETTERS(type, jtype) \
static inline j##jtype##Array Get##type##ArrayField(JNIEnv *env, jobject obj, jfieldID fid) { \
    return ubase_cast(j##jtype##Array, GetObjectArrayField(env, obj, fid)); \
} \
static inline j##jtype##Array GetStatic##type##ArrayField(JNIEnv *env, jclass cls, jfieldID fid) { \
    return ubase_cast(j##jtype##Array, GetStaticObjectArrayField(env, cls, fid)); \
}\
static inline void Set##type##ArrayField(JNIEnv *env, jobject obj, jfieldID fid, j##jtype##Array value) {\
    SetObjectArrayField(env, obj, fid, ubase_cast(jobjectArray, value));\
}\
static inline void Set##type##StaticArrayField(JNIEnv *env, jclass cls, jfieldID fid, j##jtype##Array value) {\
    SetStaticObjectArrayField(env, cls, fid, ubase_cast(jobjectArray, value));\
}

// ObjectArray
static inline jobjectArray GetObjectArrayField(JNIEnv *env, jobject obj, jfieldID fid) {
    if (obj == NULL || fid == NULL) return NULL;
    return ubase_cast(jobjectArray,_GetObjectField(env,obj,fid));
}

static inline jobjectArray GetStaticObjectArrayField(JNIEnv *env, jclass cls, jfieldID fid) {
    if (cls == NULL || fid == NULL) return NULL;
    return ubase_cast(jobjectArray,_GetStaticObjectField(env,cls,fid));
}

static inline void SetObjectArrayField(JNIEnv *env, jobject obj, jfieldID fid, jobjectArray value) {
    if (obj == NULL || fid == NULL) return;
    _SetObjectField(env, obj, fid, ubase_cast(jobject,value));
}

static inline void SetStaticObjectArrayField(JNIEnv *env, jclass cls, jfieldID fid, jobjectArray value) {
    if (cls == NULL || fid == NULL) return;
    _SetStaticObjectField(env, cls, fid, ubase_cast(jobject,value));
}

// Array
static inline jarray GetArrayField(JNIEnv *env, jobject obj, jfieldID fid) {
    return ubase_cast(jarray,GetObjectArrayField(env,obj,fid));
}

static inline jarray GetStaticArrayField(JNIEnv *env, jclass cls, jfieldID fid) {
    return ubase_cast(jarray,GetStaticObjectArrayField(env,cls,fid));
}

static inline void SetArrayField(JNIEnv *env, jobject obj, jfieldID fid, jarray value) {
    SetObjectArrayField(env,obj,fid,ubase_cast(jobjectArray,value));
}

static inline void SetStaticArrayField(JNIEnv *env, jclass cls, jfieldID fid, jarray value) {
